...
public class LotteryFeatureAnalysisServiceImpl implements ILotteryFeatureAnalysisService {
    ...
    public LotteryAdjustViewBo adjust(LLmAdjustDto dto) {
        ...
        var adjustReqBo =
                LotteryAdjustReqBo.builder().analysisReportJson(JSONObject.toJSONString(respBo)).tickets(tickets)
                        ...
                        .count(dto.getCount())
                        .userRequirement(dto.getUserRequirement())
                        .build();
    ...
    @Override
    public LotteryAdjustViewBo adjust(Integer count, boolean isTopN, String userRequirement) {
    ...
    public LotteryAnalysisRespBo doAnalyze(int sampleSize) {
        ...
        try {
            ...
            var trendAnalysis = calcTrendAnalysis(records);
            // 遗漏均线抬头红球池：合并 rising + rebounding（均由 OmissionUtils.omissionBallAnalyzer 逐球 phase 得到）
            var risingTrendRedBalls = Stream
                    .concat(CollectionUtils.emptyIfNull(trendAnalysis.getRisingRedBalls()).stream(),
                            CollectionUtils.emptyIfNull(trendAnalysis.getReboundingRedBalls()).stream())
                    .distinct().sorted().toList();
            log.info("遗漏均线抬头红球池: {}", risingTrendRedBalls);
            return LotteryAnalysisRespBo.builder().killNumbers(KillBallUtils.calculate(records))
                    ...
                    .trendAnalysis(trendAnalysis).risingTrendRedBalls(risingTrendRedBalls)
                    .featureForecast(forecastFeaturesByIndex(forecastRecords, records)).build();
        } finally {
            lock.unlock();
        }
    }
