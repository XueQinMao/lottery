...
public class OmissionUtils {
    ...
    public static TrendAnalysisVo omissionBallAnalyzer(List<HistoryRecord> historyRecords, String type, int ball) {
        ...
        List<String> periods = chronological.stream().map(HistoryRecord::getPeriod).toList();
        List<Set<Integer>> draws = chronological.stream()
                .map(r -> "red".equals(type)
                        ? Set.of(r.getNum1(), r.getNum2(), r.getNum3(), r.getNum4(), r.getNum5(), r.getNum6())
                        : Set.of(r.getSpecial()))
                .toList();
    ...
    public static PatternTrendVo omissionFeatureTrendAnalyzer(String feature, String ratio,
        ...
        Map<String, Integer> ratioCounts = LotteryFeatureTrendUtils.buckets(kind).stream()
                .collect(Collectors.toMap(Function.identity(), b -> 0, (a, b) -> a, LinkedHashMap::new));
        ...
        List<PatternTrendVo.RatioOption> options = LotteryFeatureTrendUtils.buckets(kind).stream()
                .map(optRatio -> {
                    ...
                    List<Boolean> optHitFlags = actuals.stream().map(optRatio::equals).toList();
                    ...
                    return PatternTrendVo.RatioOption.builder().ratio(optRatio).hitCount(optHits)
