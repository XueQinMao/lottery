...
/**
 * 开奖形态统计（和值 / 跨度 / 和值环比差值 / 质合比 / 奇偶比 / 尾数个数），供前端折线图展示。
 *
 * @author 刘强
 * @version 2026/08/13
 **/
...
public class FeatureStatsVo {
    ...
    /** 红球和值相对上一期增长差值（本期和值 − 上一期和值）；首期无对照，为 null */
    private List<Integer> sumDeltaValues;

    /** 红球和值环比差值均值（不含首期） */
    private Double sumDeltaAvg;
    ...
    /**
     * 红球各尾个数序列：外层下标 0–9 对应 0尾–9尾，内层按期（最旧 → 最新）。
     * 每期 6 个红球，某尾个数 ∈ [0, 6]，十个尾之和恒为 6。
     */
    private List<List<Integer>> tailCounts;

    /** 红球各尾个数均值，下标 0–9 对应 0尾–9尾 */
    private List<Double> tailAvgs;
}
