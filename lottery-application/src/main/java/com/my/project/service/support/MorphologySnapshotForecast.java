...
public final class MorphologySnapshotForecast {
    ...
    public static FeatureForecastItem forecast(String snapshotJson) {
        return forecast(snapshotJson, null);
    }

    /**
     * 按 indexValues 差值趋势确定性选出主推（不经过 LLM）。
     * <p>{@code excluded} 为需排除的取值集合（如遗漏到期概率 &gt; 0 的形态值）：
     * 主推与备选都不会落入这些取值；区间合并也会跳过被排除的桶，避免合并区间覆盖被排除值。
     */
    public static FeatureForecastItem forecast(String snapshotJson, Set<String> excluded) {
        JSONObject root = parseRoot(snapshotJson);
        List<BucketScore> scores = scoreAll(root);
...
//            .filter(s -> !isExcluded(s.ratio, excluded))
            .toList();
        BucketScore top = eligible.isEmpty()
            ? scores.stream().filter(s -> s.currentOmission > 0 && !s.rare && !isExcluded(s.ratio, excluded))
                .findFirst()
                .orElseGet(() -> scores.stream().filter(s -> !isExcluded(s.ratio, excluded))
                    .findFirst().orElse(scores.getFirst()))
            : eligible.getFirst();
        String value = maybeMergeAdjacent(feature, top, eligible.isEmpty() ? scores : eligible, excluded);
        List<String> alternatives = mergeAlternatives(feature, null, top, eligible, scores, null, value, excluded);
    ...
    /**
     * 判断某取值是否落在排除集合中（支持精确值与闭区间的相互覆盖判定）。
     */
    private static boolean isExcluded(String ratio, Set<String> excluded) {
//        if (excluded == null || excluded.isEmpty() || ratio == null || ratio.isBlank()) {
//            return false;
//        }
...
//                continue;
//            }
//            if (FeatureForecastHitUtils.matches(ratio, e) || FeatureForecastHitUtils.matches(e, ratio)) {
//                return true;
//            }
//        }
        return false;
    ...
    public static FeatureForecastItem applyGuard(FeatureForecastItem llmItem, String snapshotJson) {
        return applyGuard(llmItem, snapshotJson, null);
    }

    /**
     * 纠正 LLM 违规主推，回填 Java 计算的间隔字段，并保证长冷回补进入备选。
     * <p>{@code excluded} 为需排除的取值集合：主推命中排除集则替换为 fallback，备选也会剔除排除值。
     */
    public static FeatureForecastItem applyGuard(FeatureForecastItem llmItem, String snapshotJson,
        Set<String> excluded) {
        JSONObject root = parseRoot(snapshotJson);
        ...
        if (matched == null || forbiddenAsValue(matched) || !eligibleForValue(matched) || isExcluded(matched.ratio, excluded)) {
            reasonPrefix = "LLM主推「" + (rawValue.isEmpty() ? "空" : rawValue) + "」不合规已替换。";
            top = fallback;
        }
        String value = maybeMergeAdjacent(feature, top, eligible.isEmpty() ? scores : eligible, excluded);
        List<String> alternatives = mergeAlternatives(feature, item.getAlternatives(), top, eligible, scores, rawValue, value, excluded);
        fillItem(item, top, value, scores, alternatives);
    ...
    private static List<String> mergeAlternatives(String feature, List<String> llmAlts, BucketScore top,
        ...
        if (llmAlts != null) {
            for (String a : llmAlts) {
                tryAddAlt(alternatives, a, cover, scores, false, excluded);
            }
        }
        if (rawValue != null && !rawValue.isBlank()) {
            BucketScore rejected = matchBucket(feature, rawValue, scores);
            if (rejected != null && !rejected.rare) {
                tryAddAlt(alternatives, rejected.ratio, cover, scores, true, excluded);
            }
        }
        BucketScore mode = highestPrior(scores);
        if (mode != null) {
            tryAddAlt(alternatives, mode.ratio, cover, scores, true, excluded);
        }
        if ("ratio012".equals(feature) || "threeZone".equals(feature)) {
            tryAddAlt(alternatives, "2:2:2", cover, scores, true, excluded);
        }
        ...
        for (BucketScore s : byPrior) {
            ...
            tryAddAlt(alternatives, s.ratio, cover, scores, false, excluded);
        }
        ...
        for (BucketScore s : scores) {
            if (s.reboundWindow) {
                tryAddAlt(alternatives, s.ratio, cover, scores, true, excluded);
            }
        }
        ...
        ensureClosedSetAlts(feature, cover, alternatives, excluded);
        alternatives.removeIf(a -> coveredBy(cover, a));
        return alternatives;
    }
    ...
    private static void tryAddAlt(List<String> alternatives, String ratio, String cover,
        List<BucketScore> scores, boolean prepend, Set<String> excluded) {
        if (ratio == null || ratio.isBlank() || coveredBy(cover, ratio) || alternatives.contains(ratio)) {
            return;
        }
        if (isExcluded(ratio, excluded)) {
            return;
        }
    ...
    private static void ensureClosedSetAlts(String feature, String value, List<String> alternatives,
        ...
        for (String v : universe) {
            if (isExcluded(v, excluded) || coveredBy(value, v) || alternatives.contains(v)) {
                continue;
            }
            if (alternatives.size() < MAX_ALTERNATIVES) {
                alternatives.add(v);
    ...
    private static String maybeMergeAdjacent(String feature, BucketScore top, List<BucketScore> ranked,
        ...
        if ("sumRange".equals(feature)) {
            return maybeMergeSumRanges(top, ranked, excluded);
        }
        ...
        for (BucketScore s : ranked) {
            ...
            if (isExcluded(s.ratio, excluded)) {
                continue;
            }
            try {
                int n = Integer.parseInt(s.ratio);
    ...
    private static String maybeMergeSumRanges(BucketScore top, List<BucketScore> ranked, Set<String> excluded) {
