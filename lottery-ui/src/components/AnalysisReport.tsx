...
function killBalls(items?: { ball: number }[]) {
  return items?.map((it) => it.ball).filter((n) => n != null);
}

function KillSection({ data }: { data?: KillNumberResult }) {
  if (!data) return null;
  const hasHardRed = hasItems(data.hardKillRed);
  const hasSoftRed = hasItems(data.softKillRed);
  const hasHardBlue = hasItems(data.hardKillBlue);
  const hasSoftBlue = hasItems(data.softKillBlue);
  if (!hasHardRed && !hasSoftRed && !hasHardBlue && !hasSoftBlue && !data.basis) return null;
  return (
      ...
      <div className="chart-container">
        <div className="chart-title">
          ...
          <span className="chip-row">
            {hasHardRed && <span className="tag">必杀 {data.hardKillRed!.length}</span>}
            {hasSoftRed && <span className="tag">软杀 {data.softKillRed!.length}</span>}
          </span>
        </div>
        <div className="kv-grid">
          <div>
            <div className="kv-label">红球必杀</div>
            <BallList balls={killBalls(data.hardKillRed)} tone="kill" />
          </div>
          <div>
            <div className="kv-label">红球软杀</div>
            <BallList balls={killBalls(data.softKillRed)} tone="hot" />
          </div>
        </div>
      </div>
      <div className="chart-container">
        <div className="chart-title">
          <span>蓝球</span>
          <span className="chip-row">
            {hasHardBlue && <span className="tag">必杀 {data.hardKillBlue!.length}</span>}
            {hasSoftBlue && <span className="tag">软杀 {data.softKillBlue!.length}</span>}
          </span>
        </div>
        <div className="kv-grid">
          <div>
            <div className="kv-label">蓝球必杀</div>
            <BallList balls={killBalls(data.hardKillBlue)} tone="blue" />
          </div>
          <div>
            <div className="kv-label">蓝球软杀</div>
            <BallList balls={killBalls(data.softKillBlue)} tone="cold" />
          </div>
        </div>
      </div>
    </section>
...
function BasisHint({ text }: { text?: string }) {
  const [open, setOpen] = useState(false);
  if (!text) return null;
  return (
    <span className="basis-hint" onMouseLeave={() => setOpen(false)}>
      <button
        type="button"
        className="basis-hint-btn"
        aria-label="冷热温分档说明"
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
      >
      ...
      {open && (
        <span className="basis-hint-bubble" role="tooltip">
          {text}
        </span>
      )}
    </span>
