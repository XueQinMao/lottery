...
public class LotteryFeatureAnalysisServiceTest {
    ...
    /**
     * 这个单侧不要删除 后面要一直用 取最近 31 期，得到 30 个「样本期末 → 下一期」对照： 样本期末（如 2026066）往前 100 期算 featureForecastBo，用下一期（2026067）开奖对照命中。
     */
    @Test
    public void test_ananylsis() {
        List<HistoryRecord> window =
        ...
        for (int i = pairCount; i >= 1; i--) {
            ...
            List<HistoryRecord> past100 =
                historyRecordRepository.lambdaQuery().le(HistoryRecord::getOpenDate, end.getOpenDate())
                    .orderByDesc(HistoryRecord::getOpenDate).last("limit 100").list();
            if (CollectionUtils.size(past100) < 100) {
                System.out.printf("跳过 %s：往前不足 100 期（实际 %d）%n", end.getPeriod(), CollectionUtils.size(past100));
                continue;
            }

            var forecastRecords = past100.stream().map(this::toDrawRecord).collect(Collectors.toList());
            FeatureForecastBo forecast =
                lotteryFeatureAnalysisService.forecastFeaturesByIndex(forecastRecords, past100);
            ...
            for (int k = 0; k < kinds.length; k++) {
                ...
                if (FeatureForecastHitUtils.HIT_MAIN.equals(type)) {
                    ...
                    missDetails.add(String.format("| %s | %s | %s | %s | %s | %s |", end.getPeriod(), next.getPeriod(),
                        kind.getLabel(), actual, main, alts));
                }
            }
        ...
        for (PeriodRow row : rows) {
            ...
            md.append(' ').append(row.main).append(" | ").append(row.alt).append(" | ").append(row.miss).append(" | ")
                .append(pct(hitWithAlt, total)).append(" |\n");
        }
        ...
        for (int k = 0; k < kinds.length; k++) {
            ...
            allAlt += alt;
            allSample += n;
            md.append("| ").append(kinds[k].getLabel()).append(" | ").append(n).append(" | ").append(main).append(" | ")
                .append(pct(main, n)).append(" | ").append(alt).append(" | ").append(main + alt).append(" | ")
                .append(pct(main + alt, n)).append(" |\n");
        }
        md.append("| **合计** | ").append(allSample).append(" | ").append(allMain).append(" | ")
            .append(pct(allMain, allSample)).append(" | ").append(allAlt).append(" | ").append(allMain + allAlt)
            .append(" | ").append(pct(allMain + allAlt, allSample)).append(" |\n");
