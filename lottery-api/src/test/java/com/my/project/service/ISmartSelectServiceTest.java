...
public class ISmartSelectServiceTest {
    ...
    public void test_ananlys() {
        ...
        windows.forEach(w -> {
            List<HistoryRecord> records = historyRecordRepository.lambdaQuery()
            ...
            FeatureForecastBo featureForecastBo = lotteryFeatureAnalysisService.forecastFeaturesByIndex(forecastRecords, records);
            ...
            StringBuilder line = new StringBuilder("第" + next.getPeriod() + "期开奖" + reds + "+" + blue + " 特征命中：");
            for (FeatureKindEnums value : FeatureKindEnums.values()) {
                ...
                boolean altHit = !mainHit && item != null && item.getAlternatives() != null
                        && item.getAlternatives().stream().anyMatch(alt -> FeatureForecastHitUtils.matches(alt, actual));
                ...
                line.append("\n  ").append(value.getLabel()).append(" 实际=").append(actual)
                        .append(" 主推=").append(item == null ? null : item.getValue())
                        .append(" 备选=").append(item == null ? null : item.getAlternatives())
                        .append(" ").append(type);
            }
            System.out.println("特征featureForecastBo = "+JSON.toJSONString(featureForecastBo));
            System.out.println(line);
            System.out.println("第" + next.getPeriod() + "期命中统计：主推" + mainCnt + " 备选" + altCnt
                    + " 未命中" + missCnt + " 含备选命中率" + pct(mainCnt + altCnt, mainCnt + altCnt + missCnt));
        });
    }
