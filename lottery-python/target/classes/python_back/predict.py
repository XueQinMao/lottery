...
def fuse_prediction(feat_vec, seq_vec, state_id):
    ...
    1. 并行调用所有子模型，收集概率
    2. 按元特征顺序组装 X_meta
    3. 用 second_fusion_model 预测最终概率
    """
    ...
    # 并行调用所有子模型
    prob_dict = {}
    with ThreadPoolExecutor(max_workers=len(meta_names)) as executor:
        futures = {}
    ...
    # 按 meta_names 原始顺序收集概率，保持元特征列一致
    meta_probas = []
    used = []
    for name in meta_names:
...
def handle_auto_predict(request):
    ...
    try:
        ...
        for reds, blue in candidates:
            ...
            prob, _ = fuse_prediction(feat_vec, seq_vec, state_id)  # 此处使用并行融合
