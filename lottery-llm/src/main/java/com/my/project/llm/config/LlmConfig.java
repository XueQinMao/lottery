    ...
    public ChatClient lotteryAdjustChatClient(ChatModel chatModel) {
        return ChatClient.builder(chatModel)
                .defaultSystem("""
                        ...
                        以及 featureForecast 形态先验——含备选命中高主要来自覆盖面，不是开奖答案；
                        dueWindow/confidence 不是命中率，禁止按它们给 value 加权；
                        覆盖已满的维不作过滤；cooling+刚出或 eta≤-3 勿追 value；
                        比例维一组偏均衡、一组走另一侧备选；禁止 15 维同时硬锁、禁止所有组追逐同一套 value）；
                        用户消息会明确本次是调优还是推荐。
