...
public final class LotteryAdjustPrompt {
    ...
    public static final String USER_PROMPT = """
            ...
            8. 【趋势均线补充约束】若报告存在 `trendAnalysis` 字段：
            ...
            9. 【极端整组重调】默认以局部替换为主；但若本组原号码陷入极端不合规，
               **允许将红球 6 个与蓝球全部替换**，等价于按报告从零重选一组单式。
               ...
               重调后的号码仍须满足全部硬约束（杀号/冷热/和值/跨度/分区/连号等）。
            ...
            2. 红球替换：红球 1-33 互异升序共 6 个。在维持合理形态前提下，按优先级处理：
               ...
               (h) 【极端整组重调】若局部替换（通常 1-2 个）仍无法同时满足杀号/冷热/分区/连号/
                   和值/跨度等硬约束，允许一次性替换全部 6 个红球（可同时替换蓝球），
                   按报告约束重新生成合规单式；优先从 `trendAnalysis.risingRedBalls` 与温号池选取，
